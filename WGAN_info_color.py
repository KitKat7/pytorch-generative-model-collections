...
class WGAN_info_color(object):
    def __init__(self, args):
        ...
        if self.dataset == 'mnist':
            ...
            self.data_loader = utils.load_sdd('../processed/bounding_box_imgs', transform=transforms.Compose(
        ...
        x_dim = self.vis_size
        y_dim = len_discrete_code
        z_dim = self.z_dim
        sample_num = x_dim * y_dim
        
        self.sample_z_ = torch.zeros((sample_num, z_dim))
        for i in range(x_dim):
            self.sample_z_[i * y_dim] = torch.rand(1, z_dim)
            for j in range(1, y_dim):
                self.sample_z_[i * y_dim + j] = self.sample_z_[i * y_dim]
                
        temp = torch.zeros((y_dim, 1))
        ...
        self.sample_y_ = torch.zeros((sample_num, y_dim))
        self.sample_y_.scatter_(1, temp_y.type(torch.LongTensor), 1)  # diagonal
        self.sample_c_ = torch.zeros((sample_num, len_continuous_code))
        
        # manipulating two continuous code
        z_dim2 = self.z_dim
        sample_num2 = self.vis_size * self.vis_size

        temp_z_ = torch.rand((1, z_dim2))
        self.sample_z2_ = temp_z_

        for i in range(sample_num2 - 1):
            self.sample_z2_ = torch.cat([self.sample_z2_, temp_z_])

        y = np.zeros(sample_num2, dtype=np.int64)
        y_one_hot = np.zeros((sample_num2, len_discrete_code))
        y_one_hot[np.arange(sample_num2), y] = 1
        self.sample_y2_ = torch.from_numpy(y_one_hot).type(torch.FloatTensor)

        temp_c = torch.linspace(-1, 1, self.vis_size)
        self.sample_c2_ = torch.zeros((sample_num2, len_continuous_code))
        
        # import IPython
        # IPython.embed()
        
        for i in range(self.vis_size):
            for j in range(self.vis_size):
                self.sample_c2_[i * self.vis_size + j, 0] = temp_c[i]
                self.sample_c2_[i * self.vis_size + j, 1] = temp_c[j]
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                self.writer.add_image('Fake_disc_fixed_image', vutils.make_grid(samples.data, nrow=len_discrete_code), step)

                samples = self.G(self.sample_z2_, self.sample_c2_, self.sample_y2_)
                self.writer.add_image('Fake_cont_fixed_image', vutils.make_grid(samples.data, nrow=self.vis_size), step)

                if ((iter + 1) % 100) == 0:
                    print("Epoch: [%2d] [%4d/%4d] D_loss: %.8f, G_loss: %.8f, info_loss: %.8f" %
                          ((epoch + 1), (iter + 1), self.data_loader.dataset.__len__() // self.batch_size, D_loss.data[0], G_loss.data[0], info_loss.data[0]))
